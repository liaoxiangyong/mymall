...
import edu.ccnt.mymall.dao.*;
import edu.ccnt.mymall.model.*;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId,Integer shippingId){
        log.info("创建订单");
        List<Cart> cartList = cartMapper.selectByUserIdAndChecked(userId); //获取购物车中的商品信息
        ...
        //计算订单总价
        BigDecimal payment = this.countTotalPrice(orderItemList);
    ...
    private OrderItemVo assembleOrderItemVo(OrderItem orderItem){       //生成orderItemVo返回对象
    ...
    private ShippingVo assembleShippingVo(Shipping shipping){      //生成shippingVo返回对象
    ...
    private void cleanCart(List<Cart> cartList){     //订单生成成功以后清空购物车
        for(Cart cart : cartList){
            cartMapper.deleteByPrimaryKey(cart.getId());
        }
    }

    private void reduceProductStock(List<OrderItem> orderItemList){     //订单生成成功需要减少产品库存
        for(OrderItem orderItem : orderItemList){
    ...
    private Order assembleOrder(Integer userId,Integer shippingId,BigDecimal payment){//生成订单
    ...
    private ServerResponse creteOrderItem(Integer userId,List<Cart> cartList){        //根据购物车创建订单信息
        List<OrderItem> orderItemList = Lists.newArrayList();
    ...
    private BigDecimal countTotalPrice(List<OrderItem> orderItemList ){ //计算订单总价
        BigDecimal payment = new BigDecimal("0");
        for(OrderItem orderItem : orderItemList){
            payment = BigDecimalUtil.add(payment.doubleValue(),orderItem.getTotalPrice().doubleValue());
        }
    ...
    public ServerResponse pay(Integer userId,Long orderNo,String path){
        ...
        String subject = new StringBuilder().append("mymall在线商城扫描支付，订单号：").append(outTradeNo).toString();
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setTimeoutExpress(timeoutExpress)
                 .setNotifyUrl(PropertiesUtil.getProperty("alipay.url"))//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
                .setGoodsDetailList(goodsDetailList);

        /** 一定要在创建AlipayTradeService之前调用Configs.init()设置默认参数
         *  Configs会读取classpath下的zfbinfo.properties文件配置信息，如果找不到该文件则确认该文件是否在classpath目录
         */
        Configs.init("src/main/resources/zfbinfo.properties");
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        if(order == null){
            return ServerResponse.createByErrorMessage("mymall商城没有该订单");
        }
